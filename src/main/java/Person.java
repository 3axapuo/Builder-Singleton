...
// ЗАДАЧА №1, cоздайте класс Person с полями, необходимыми для хранения данных, указанных в условии.
public class Person {
    ...
    // ЗАДАЧА №1 также в класс Person надо добавить метод PersonBuilder newChildBuilder(), который будет возвращать
    // полузаполненный билдер для ребёнка, а именно:
    ...
    public boolean hasAddress() { // ЗАДАЧА №1, текущий город жительства (String). Может быть известен (в этом случае метод
        if (city != null && !city.isEmpty()) return true; // должен вернуть true
    ...
    public void happyBirthday() { // ЗАДАЧА №1, если возраст человека известен, то с момента создания объекта он
        if (hasAge()) age += 1; //может быть изменён только увеличением на единицу через вызов метода happyBirthday()
    }
